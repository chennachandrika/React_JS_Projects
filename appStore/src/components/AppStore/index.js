...
class AppStore extends Component {
  state = {activeTabId: tabsList[0].tabId, searchText: ''}
  ...
  render() {
    ...
    const visibleAppItems = appsList.filter(
      appListItem =>
        appListItem.category === activeTabId &&
        appListItem.appName.toLowerCase().includes(searchText.toLowerCase()),
    )
    return (
      <div className="app-container">
        ...
        <ul className="app-store-items">
          {visibleAppItems.map(appListItem => (
            <AppItem key={appListItem.appId} appItem={appListItem} />
          ))}
        </ul>
      </div>
    )
